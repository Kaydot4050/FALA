...
export default function Home() {
  ...
  return (
    <div className="flex flex-col gap-10 pb-28 md:pb-12">
      ...
      {/* ── Hero ── */}
      <section className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary/90 via-primary to-violet-700 text-primary-foreground px-6 py-10 md:px-12 md:py-14 mt-2">
        {/* decorative blobs */}
        ...
        <div className="relative z-10 max-w-xl">
          ...
          <p className="text-base md:text-lg text-white/80 leading-relaxed">
            Pick your network, choose a bundle, enter your number — data lands in seconds.
            No account needed.
          </p>
        </div>
          ...
          {NETWORKS.map((net) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "relative flex flex-col items-center gap-3 p-5 rounded-2xl border-2 transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2",
                  isSelected
                    ? `border-transparent bg-primary/10 ring-2 ${net.ringColor} shadow-md`
                    : "border-border bg-card hover:border-border/0 hover:shadow-sm hover:bg-muted/60"
                )}
              >
                ...
                {isSelected && (
                  <div className="absolute top-2.5 right-2.5 w-2.5 h-2.5 rounded-full bg-primary" />
                )}
              </button>
            );
          })}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {packages.map((pkg) => (
              <button
                ...
                className="group flex flex-col items-start text-left p-5 rounded-2xl border border-border bg-card shadow-sm hover:shadow-md hover:border-primary/40 hover:-translate-y-0.5 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
              >
