...
const NETWORKS: { id: NetworkId; name: string; icon: React.ElementType; colorClass: string; bgClass: string }[] = [
  { 
    ...
    name: "MTN", 
    icon: Zap, 
    colorClass: "text-[#ffcc00] dark:text-[#ffcc00]", 
  ...
  { 
    ...
    name: "Telecel", 
    icon: WifiHigh, 
    colorClass: "text-[#e60000] dark:text-[#ff3333]", 
  ...
  { 
    ...
    name: "AT", 
    icon: Radio, 
    colorClass: "text-[#008080] dark:text-[#00aaaa]", 
    bgClass: "bg-[#008080]/10 hover:bg-[#008080]/20 border-[#008080]/20 hover:border-[#008080]/50" 
  },
];
...
export default function Home() {
  ...
  const { data: packagesRes, isLoading, isError } = useGetDataPackages({ network: selectedNetwork });
  
  const packages = packagesRes?.data?.[selectedNetwork] || [];
    ...
    <div className="flex flex-col space-y-8 pb-12">
      <section className="py-8 md:py-12 space-y-4 max-w-2xl">
        ...
        <p className="text-lg md:text-xl text-muted-foreground leading-relaxed">
          Select your network, pick a package, and get connected instantly. No hassle, just speed.
        </p>
      </section>
      ...
      <section className="space-y-6">
          ...
          <div className="grid grid-cols-3 gap-3 md:gap-6">
            {NETWORKS.map((net) => {
              ...
              return (
                ...
                >
                  {/* Subtle brand colored glow behind icon when unselected */}
                  {!isSelected && (
                    <div className={cn("absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500", net.bgClass)} />
                  )}
                  
                  <div className={cn("relative z-10 flex flex-col items-center gap-3")}>
                    <div className={cn(
                      "p-3 rounded-full",
                    ...
                    <span className="font-bold text-sm md:text-base tracking-tight">{net.name}</span>
                  </div>
                </button>
              );
            })}
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            {isLoading && (
              <span className="text-xs font-medium text-muted-foreground flex items-center animate-pulse">
                Loading packages...
              </span>
            )}
          </div>
          ...
          {isError ? (
            <div className="bg-destructive/10 text-destructive p-6 rounded-xl border border-destructive/20 text-center">
              ...
              <p className="text-sm mt-1 opacity-80">Please try selecting a network again.</p>
            </div>
          ) : isLoading ? (
                ...
                >
                  <div className="w-full flex justify-between items-start mb-4">
                    <div className="flex items-end gap-1">
                      <span className="text-3xl font-extrabold tracking-tight leading-none group-hover:text-primary transition-colors">{pkg.capacity}</span>
